class Profile {
    constructor() {
        ...
        // Автозаполнение из Telegram WebApp, если доступно
        try {
            const tg = window.Telegram && window.Telegram.WebApp;
            if (tg && tg.initDataUnsafe && tg.initDataUnsafe.user) {
                const u = tg.initDataUnsafe.user;
                if (u.first_name && !this.userData.firstName) this.userData.firstName = u.first_name;
                if (u.last_name && !this.userData.lastName) this.userData.lastName = u.last_name;
                // Телефон Telegram Mini Apps передаёт только при авторизации через Login Widget/бот —
                // если есть, подхватим из DataService/userData.phone, иначе оставим текущее значение
                if (!this.userData.phone && DataService?.userData?.phone) this.userData.phone = DataService.userData.phone;
                this.saveUserData();
            }
        } catch (_) {}
    }
            ...
            <div class="profile-header fade-in">
                ...
                <div class="profile-level">
                    ...
                    <div class="bonus-points">
                        <span class="bonus-count">${Number(this.userData.bonusPoints).toLocaleString()}</span>
                                ...
                                <div class="order-item">
                                    ...
                                    <div class="order-items">
                                     ...
                                     <div style="margin-top:8px; display:flex; gap:8px;">
                                       ...
                                       <button class="btn-primary" data-view-order="${order.id}">Подробнее</button>
                                     </div>
                                </div>
                            `).join('')}
                        </div>
          ...
          <div class="modal-window">
            ...
            <div class="modal-body">
              <div class="order-meta">
                <div><strong>Дата:</strong> ${new Date(order.date).toLocaleString()}</div>
                <div><strong>Статус:</strong> ${this.getOrderStatusText(order.status||'new')}</div>
              </div>
                ...
                ${order.items.map(i=>`
                  <div class="modal-item">
                    ...
                    <div class="mi-info">
                      ...
                      <div class="mi-meta">${(i.price||0).toLocaleString()} ₽ × ${i.quantity||1}</div>
                    </div>
                    <div class="mi-sum">${((i.price||0)*(i.quantity||1)).toLocaleString()} ₽</div>
                  </div>
                `).join('')}
              </div>
            ...
            <div class="modal-footer">
              <button class="btn-add" data-reorder-id="${order.id}">Повторить</button>
              <button class="btn-primary modal-close">Закрыть</button>
            </div>
          </div>`;
        document.body.appendChild(overlay);
        const close = ()=> overlay.remove();
        overlay.addEventListener('click', (e)=>{ if(e.target.classList.contains('modal-overlay')) close(); });
        overlay.querySelectorAll('.modal-close').forEach(b=> b.addEventListener('click', close));
        // Повторить из модалки
        overlay.querySelector('[data-reorder-id]')?.addEventListener('click', ()=>{
          window.cart.items = order.items.map(i=>({ id:i.id, title:i.title, price:i.price, image:i.image||'', quantity:i.quantity||1 }));
          window.cart.saveToStorage();
          Router.navigate('cart');
          close();
